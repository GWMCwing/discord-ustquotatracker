...
export class UstController {
  ...
  static generateNotification(logEntries: logEntry[]): NotificationMap {
    ...
    for (let i = 0; i < logEntries.length; i++) {
      ...
      notificationMap.get(entry.courseCode)?.set(entry.section, entry.log);
    }
  ...
  static async generateUserNotification(
    ...
    const userSection_Cursor = userDb.getUser_Section(semester, sectionCode);
  ...
  static async sendToChannel_UserNotification(
    ...
    let text = '';
    let count = 0;
    const chList = await bot.getUstTextChannel(dept);
    ...
    for (let [courseCode, sectionEntry] of notificationMap) {
      for (let [sectionCode, log] of sectionEntry) {
        userNotificationPromise.push(
          this.generateUserNotification(userNotificationMap, dept, courseCode, sectionCode, log)
        );
        text += `${log}\n`;
        count++;
        if (text.length >= 1500) {
          for (let ch of chList) bot.sendMessage_channel(ch, text, dept, count);
          text = '';
          count = 0;
        }
      }
  ...
  static async sendToUser(bot: Bot, dept: deptName, userNotificationMap: UserNotificationMap) {
  ...
  static async sendLog_dev(dept: deptName, logEntries: logEntry[]) {
    ...
    await this.sendToChannel_UserNotification(bot, dept, notificationMap, userNotificationMap);
  ...
  static async updateSectionQuota(
    ...
    if (document === null) {
      // the classId is new
      CLL.log(threadName, 'Section Update', `New Section: ${sectionQuota.courseCode}`);
    ...
    return `🍕 Quota Changed: ${courseCode} - ${title} [${sectionQuota.section}(${sectionQuota.classId})] (${
      document.quota
    } -> ${sectionQuota.quota} (${changes > 0 ? '+' + changes : changes})).`;
  }
  static async updateSubject(dept: deptName) {
    ...
    for (const [courseCode, { title, sectionQuotas }] of [...data.entries()]) {
      ...
      for (const sectionQuota of sectionQuotas) {
        const str = await this.updateSectionQuota(sectionQuota, courseCode, title);
        if (str)
          logEntries.push({
  ...
  public static async update() {
    ...
    /** // commented due to repeated declaration of the string tuple
        ...
        */
    const subjects: Array<deptName> = Object.keys(configs.discord.ustChIds) as unknown as Array<deptName>;
    //
    for (const subject of subjects) {
      ...
      CLL.log(threadName, 'Course Update', `${subject} Course Quota Updated...`);
    }
